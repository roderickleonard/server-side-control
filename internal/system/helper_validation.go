package system

import "errors"

const MaxHelperPayloadBytes = 1 << 20

var ErrHelperActionNotAllowed = errors.New("helper action not allowed")

var allowedHelperActions = map[string]struct{}{
	"user.create":                   {},
	"user.list":                     {},
	"user.delete":                   {},
	"user.set_password":             {},
	"user.set_sudo":                 {},
	"user.set_passwordless_sudo":    {},
	"site.fix_laravel_permissions":  {},
	"mysql.provision_database":      {},
	"mysql.list_access":             {},
	"mysql.delete_access":           {},
	"mysql.rotate_user_password":    {},
	"mysql.rotate_admin_password":   {},
	"mysql.inspect_database":        {},
	"mysql.restore_database":        {},
	"mysql.execute_query":           {},
	"mysql.inspect_service":         {},
	"mysql.configure_service":       {},
	"mysql.install_service":         {},
	"mysql.upgrade_service":         {},
	"mysql.start_service":           {},
	"mysql.stop_service":            {},
	"mysql.restart_service":         {},
	"mysql.service_logs":            {},
	"mysql.execute_admin_query":     {},
	"mysql.export_database":         {},
	"nginx.apply_site":              {},
	"nginx.delete_site":             {},
	"nginx.validate":                {},
	"nginx.reload":                  {},
	"nginx.enable_tls":              {},
	"nginx.write_config":            {},
	"nginx.validate_config":         {},
	"nginx.set_config_enabled":      {},
	"deploy.run":                    {},
	"deploy.run_custom_git_command": {},
	"deploy.rollback":               {},
	"deploy.inspect":                {},
	"runtime.inspect":               {},
	"runtime.install_nvm":           {},
	"runtime.install_node":          {},
	"runtime.install_pm2":           {},
	"runtime.install_composer":      {},
	"runtime.start_pm2":             {},
	"runtime.run_npm_script":        {},
	"runtime.run_npm_install":       {},
	"runtime.run_custom_command":    {},
	"runtime.run_shell_command":     {},
	"panel.write_env":               {},
	"panel.apply_proxy":             {},
	"panel.restart_service":         {},
	"panel.inspect_tls":             {},
	"files.write_env":               {},
	"files.read_env":                {},
	"files.read_text":               {},
	"files.write_text":              {},
	"files.list_dir":                {},
	"files.clear_directory":         {},
	"git_auth.inspect":              {},
	"git_auth.ensure_deploy_key":    {},
	"git_auth.trust_host":           {},
	"git_auth.store_credential":     {},
	"pm2.list":                      {},
	"pm2.restart":                   {},
	"pm2.reload":                    {},
	"pm2.start":                     {},
	"pm2.stop":                      {},
	"pm2.logs":                      {},
	"php.switch":                    {},
	"php.list_versions":             {},
	"php.list_installable_versions": {},
	"php.install_versions":          {},
	"php.list_extension_status":     {},
	"php.install_extensions":        {},
	"php.enable_extensions":         {},
	"php.disable_extensions":        {},
	"php.diagnostics":               {},
	"php.read_ini_settings":         {},
	"php.update_ini_settings":       {},
	"redis.inspect":                 {},
	"redis.install":                 {},
	"redis.configure":               {},
	"redis.start":                   {},
	"redis.stop":                    {},
	"redis.restart":                 {},
	"redis.test_connection":         {},
	"redis.logs":                    {},
	"supervisor.inspect":            {},
	"supervisor.install":            {},
	"supervisor.start_service":      {},
	"supervisor.stop_service":       {},
	"supervisor.restart_service":    {},
	"supervisor.reread":             {},
	"supervisor.update":             {},
	"supervisor.list_programs":      {},
	"supervisor.save_program":       {},
	"supervisor.remove_program":     {},
	"supervisor.start_program":      {},
	"supervisor.stop_program":       {},
	"supervisor.restart_program":    {},
	"supervisor.tail_logs":          {},
	"cron.list":                     {},
	"cron.create":                   {},
	"cron.update":                   {},
	"cron.delete":                   {},
	"cron.clear_log":                {},
	"cron.rotate_log":               {},
	"ssh_account.inspect":           {},
	"ssh_account.add_key":           {},
	"ssh_account.remove_key":        {},
	"ssh_account.set_password":      {},
	"backup.run_site":               {},
	"backup.prune_site":             {},
	"firewall.status":               {},
	"firewall.enable":               {},
	"firewall.disable":              {},
	"firewall.add_rule":             {},
	"firewall.delete_rule":          {},
}

func ValidateHelperAction(action string) error {
	if _, ok := allowedHelperActions[action]; !ok {
		return ErrHelperActionNotAllowed
	}
	return nil
}
